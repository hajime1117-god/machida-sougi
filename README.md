...
### 第4弾（ファーストビューの全面リニューアル）

- 想定ユーザー（60〜80代の女性・スマートフォン・葬儀は初めて・急いでいる）の不安を減らすことを最優先に FV を作り直し
- 構成: 写真（白い花と自然光。Pexels）→ 地域ラベル → 見出し「突然のことでも、お電話一本で大丈夫です。」→ 3つの約束（一つずつ案内／費用を先に説明／お迎えだけの依頼可）→ 大きな電話CTA（番号・24時間365日・相談無料）→ 副CTA「費用の目安を見る」→ 状況別リンク → 「お電話→お迎え・安置→落ち着いてから決める」の3ステップ
- 本文 17px 以上、CTA 高さ 58px 以上、薄いグレー文字を排除。SP はヘッダーにも「電話で相談」の文字を表示
- PC は写真を背景に左から淡いオーバーレイをかけて可読性を確保（黒オーバーレイは不使用）
- 「ご希望から選ぶ」は FV 直下の独立セクションに移動。見出し・内部リンク・計測属性は維持。Lighthouse Mobile 99 / Desktop 100
